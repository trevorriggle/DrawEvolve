...
//
// JWT integration points are marked with [JWT-INTEGRATION] — App Attest
// runs alongside but never replaces JWT validation. Both must pass.
//
...
const APP_ATTEST_KEY_TTL_SECONDS = 30 * 24 * 3600;   // 30d sliding expiration on stored keys
const APP_ATTEST_CHALLENGE_TTL_SECONDS = 5 * 60;     // 5m to complete attestation
const APP_ATTEST_CHALLENGE_BYTES = 32;
...
function ecdsaDerToRaw(der, curveByteLength) {
 ...
 */
async function verifyAppAttestAttestation({ keyIdB64, attestationB64, challengeBytes, env }) {
 ...
 */
async function verifyAppAttestAssertion({ assertionB64, storedPubKey, storedCounter, expectedClientDataHash, env }) {
 ...
 */
async function computeAppAttestClientDataHash(method, path, bodyBytes) {
...
async function issueAppAttestChallenge(env) {
...
async function consumeAppAttestChallenge(challengeBytes, env) {
...
async function storeAttestedKey(keyIdB64, publicKey, env) {
  if (!env.QUOTA_KV) throw new Error('QUOTA_KV not bound');
...
async function getAttestedKey(keyIdB64, env) {
...
async function updateAttestedKeyCounter(keyIdB64, newCounter, env) {
 ...
 */
function readAppAttestHeaders(request) {
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Phase 5f — App Attest endpoints. These run BEFORE JWT validation
    // intentionally: registration must be reachable on first launch when
    // the device hasn't yet been associated with the user's session token
    // refresh schedule, and the rate-limit + body-size gates inside each
    // handler bound abuse.
    //
    // [JWT-INTEGRATION] Once the parallel JWT work lands, decide whether
    // /attest/register should additionally require a valid JWT. The current
    // design tolerates either choice — keyId is bound to the device, not
    // the user, so JWT-less registration is safe but ties the device to
    // whoever first registers it. Adding JWT here gates that to authed
    // users only.
    ...
    if (url.pathname === '/attest/register') {
      try {
        ...
        let challengeBytes;
        try { challengeBytes = base64ToBytes(challenge); }
    ...
    try {
      ...
      // who's calling) and before the rest of validation (so unattested
      // devices can't probe other endpoint behavior). Failures return 401
      // with a stable error code — clients use the code to decide whether
      // to wipe their cached keyId and re-register.
      ...
      if (!stored) {
        ...
        // Returning a distinct code lets the client wipe + re-register
        // without confusing this with a JWT failure.
        ctx.waitUntil(logRequest({ env, status: REQUEST_STATUS.AUTH_FAILED, userId, ipHash }));
...
export {
  ...
  REQUEST_STATUS,
  logRequest,
  // Phase 5f — App Attest (exposed for tests)
  cborDecode,
  parseX509,
  spkiPublicKeyBytes,
  findExtensionValue,
  ...
  APP_ATTEST_OID,
  APP_ATTEST_AAGUID_DEV,
  APP_ATTEST_AAGUID_PROD,
};
