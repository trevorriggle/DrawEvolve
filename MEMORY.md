...
## Two summary types live in disjoint namespaces

There are TWO unrelated "summary" artifacts in the system. They look superficially similar and would silently drift if conflated, so be deliberate about which one you're touching.

**Conversation-level summaries** (Eve's within-conversation memory):
- Source of truth: `conversations.rolling_summary` (migration `0020_eve_rolling_summary.sql`).
- Generated by `lib/eve-summary.js` (`gpt-5-mini`, ~400-word prose) via `routes/eve.js`'s `maybeRegenerateRollingSummary`, fired post-turn via `ctx.waitUntil`.
- Read into Eve's prompt by `lib/eve-prompt.js`'s CONVERSATION SO FAR block (`renderRollingSummaryBlock`).
- Regenerates as the conversation grows; the boundary advances forward.

**Critique summaries** (cross-drawing portfolio context):
- Source of truth: trailing `<!--summary-->...<!--/summary-->` block in `drawings.critique_history[].content`, written by the critique model itself as part of its critique output.
- Parsed by `lib/critique-summary.js`'s `parseCritiqueSummary` (no separate model call — the bullets are extracted from text that already exists).
- Read into Eve's prompt by `lib/supabase.js`'s `fetchCoachingContext` → `lib/eve-prompt.js`'s `renderCoachingSummaryRow`, surfaced as part of the COACHING CONTEXT block under "Recent critique summaries (newest first):".
- Frozen at critique-write time; never regenerates.

**The two systems summarize different artifacts** (a single critique markdown body vs. a multi-turn Eve conversation) and are not interchangeable. There is no other conversation-summarization path. **Future cross-conversation continuity features (e.g., "let Eve reference threads from prior conversations") must project `rolling_summary` from prior conversations via `listConversations`-style reads — do not build a second conversation-summary layer.**

If a future contributor proposes summarizing conversations from a new prompt or schema, the answer is: extend `lib/eve-summary.js` and/or bump `rolling_summary_version` (which lets `renderRollingSummaryBlock` branch on schema), not parallel implementation.

See: rolling-summary design proposal at `~/Desktop/eve-rolling-summary-proposal.md` (May 2026) §8.
...
## 2026-06 wave — canvas rework, ghost layer, perf (decisions that aren't obvious from code)

As of 2026-06-11, shipped on `main` (see git log 5.1–5.5 + fix commits). The non-obvious rules each change established:

- **Keyboard avoidance pans the canvas via `effectivePanOffset`** (type-tool rework 5.1). The transient `keyboardAvoidancePan` is a SEPARATE field composed at presentation time — gesture math (anchored zoom/rotate, two-finger pan) reads/writes raw `panOffset` and must never see it. Every presentation consumer (documentToScreen, screenToDocument wrappers, both Metal draw snapshots) reads `effectivePanOffset`. The old "transient pan corrupts pinch-zoom state" objection (TextEntryOverlay's retired preview card) is solved by this separation, not disproven.
- **Wet-ink `.max/.max` deposit makes per-stamp pressure terms a per-stroke value CEILING** — overlap cannot rebuild what a pressure discount removes. That was the "airbrush needs hard Pencil pressure" bug; airbrush/watercolor pressure tapers are deliberately shallow (0.85 floor). Corollary: position-keyed shader modulation (layer-space streaks/grain) survives max-blend; per-stamp radial features (edge rims) wash out to the stroke cross-section — design stamp profiles AS the desired cross-section.
- **`BrushUniforms.strokeDir` (5.4) landed in tail padding** — float2 at offset 56, struct stride stays 64 on both MSL and Swift sides, so the C-ABI prefix is unchanged for shaders that don't read it. Future fields: trailing-append only, re-verify layout math.
- **Every async-committed Metal stroke command buffer must hold a `strokeCommandSlots` slot.** The parked blur-brush crash (EXC_BREAKPOINT on com.Metal.CommandQueueDispatch) was the real-time blur path skipping this and blowing past the ~64-buffer per-queue cap under 240 Hz input.
- **The canvas MTKView is event-driven for life** (`isPaused = true` + `enableSetNeedsDisplay = true`). `isPaused = false` shipped for weeks and silently defeated the entire snapshot-gate/throttle architecture — draw() free-ran at 120 Hz on idle canvas (the iPad-hot bug). A stale frame after a state change means a field is missing from `CanvasRenderSnapshot`; add the field, never un-pause. The foreground observer deliberately only fires one setNeedsDisplay.
- **Ghost layer (annotations) mirrors the classifier contract exactly**: second gpt-5-mini call (vision), runs in PARALLEL with the tag classifier before persistCritique, strict json_schema, ≥0.5 confidence filter, null on any failure — markers are garnish, never block a critique. Stored as a plain array on the critique_history entry (JSONB ⇒ no migration). Grounding accuracy was NOT validated pre-ship (deliberate "ship it" call); first tuning knobs are MIN_ANNOTATION_CONFIDENCE and the system prompt in `lib/annotations.js`.
- **Replay Tutorial is a counter, not a flag edge** (`tutorialReplayRequestV1`): resetting an already-false `hasSeenTutorialV1` produced no onChange and left the button dead. Cross-view requests through AppStorage should be monotonic tokens.
- **Model names:** critiques + Eve chat = `gpt-5.1` (`reasoning_effort: 'none'`, flat); classifier + annotator + Eve summaries = `gpt-5-mini` (`reasoning_effort: 'minimal'`, flat; needs `max_completion_tokens` ≈ 2000 because reasoning tokens count). `gpt-5.1-mini` does not exist and 404s silently.
- **Docs reorg:** superseded April–May plans/audits moved to `docs/archive/` (incl. both custom-prompts plans and the April 27th doc). RATELIMITSPLAN.md and ONLINEIMPLEMENTATIONPLANS.md stay at root — they're unbuilt future work, not history.
