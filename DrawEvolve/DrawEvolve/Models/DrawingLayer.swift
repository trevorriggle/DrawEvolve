...
class DrawingLayer: Identifiable, ObservableObject {
    ...
    /// Sparse tile-grid storage for this layer's pixels. Nil until the layer
    /// is initialized (renderer-allocated via `makeEmptyTileGrid()` at layer
    /// creation / load).
    ///
    ...
    /// allocated tiles into a new grid.
